...
get_current_branch() {
    ...
    # Use symbolic-ref first so a repository with no commits still reports the
    # checked-out branch instead of failing with "ambiguous argument 'HEAD'".
    ...
    if has_git; then
        ...
        if branch=$(git -C "$repo_root" rev-parse --abbrev-ref HEAD 2>/dev/null) && [[ -n "$branch" && "$branch" != "HEAD" ]]; then
            echo "$branch"
            return
        fi
    fi
...
check_feature_branch() {
    # Branch naming validation is disabled for this project. Feature resolution
    # relies on .specify/feature.json, SPECIFY_FEATURE_DIRECTORY, or the legacy
    # branch-name fallback without failing commands because of Git branch state.
    return 0
}
