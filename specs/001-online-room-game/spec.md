...
- **服务端权威与规则一致性**: 服务端负责发牌、阶段推进、身份行动、交换、投票、结算和房间清理；规则来源为 `game_rule.md` 与本规格。
- **临时数据与隐私最小化**: 仅临时保存昵称、房间/对局状态、重连令牌数据和实时聊天转发数据；结算或清理后不保留服务端对局或聊天历史。
- **测试先行与契约可验证**: 规则、可见性、HTTP/Socket.IO 契约、重连、无效操作、投票和端到端三浏览器流程均要求自动化测试覆盖。
- **简洁自托管架构**: 第一版限定单体 TypeScript Web 应用、内存房间存储、邀请制房间、极简文字/CSS UI，无账号、公开大厅、匹配、旁观者或自定义美术。
- **清晰中文体验与实时可用性**: 玩家界面、错误反馈和非技术说明默认中文；可见状态更新目标为 95% 在 2 秒内到达受影响在线客户端。
